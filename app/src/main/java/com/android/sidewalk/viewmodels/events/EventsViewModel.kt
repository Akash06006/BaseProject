...
class EventsViewModel : BaseViewModel() {
    ...
    private var eventDetail =
        MutableLiveData<EventDetailResponse>()
    ...
    init {
        if (UtilsFunctions.isNetworkConnectedReturn()) {
            addUpdateTruck = eventsRepository.addUpdateImage(null, null)
            eventsList = eventsRepository.eventsList("0")
            eventDetail = eventsRepository.eventDetail(null)
        }
    ...
    fun getEventDetail() : LiveData<EventDetailResponse> {
        return eventDetail
    }
    ...
    fun eventList(type : String) {
        if (UtilsFunctions.isNetworkConnected()) {
            eventsList = eventsRepository.eventsList(type)
    ...
    fun eventDetail(id : String) {
        if (UtilsFunctions.isNetworkConnected()) {
            eventDetail = eventsRepository.eventDetail(id)
            mIsUpdating.postValue(true)
        }
    ...
    fun viewGallery(id : String) {
        if (UtilsFunctions.isNetworkConnected()) {
            viewGallery = eventsRepository.viewGallery(id)
            mIsUpdating.postValue(true)
        }
